# What is the address of the P2SH script wrapping the witness redeem script in this transaction
transaction=""

# Step 1: Decode the raw transaction using bitcoin-cli
decoded_tx=$(bitcoin-cli -regtest decoderawtransaction "$transaction")

# Step 2: Extract the scriptSig.asm field from the first input using jq
script_sig=$(echo "$decoded_tx" | jq -r '.vin[0].scriptSig.asm')

# Step 3: Get the second field (redeem script) from the scriptSig
redeemScript=$(echo "$script_sig" | awk '{print $NF}')

# Step 4: Decode the redeem script to get the address
address=$(bitcoin-cli -regtest decodescript "$redeemScript" | jq -r '.segwit["p2sh-segwit"]')

# Output the address
echo "$address"